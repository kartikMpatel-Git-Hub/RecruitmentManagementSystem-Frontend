...
import { Users, GraduationCap, Award, Briefcase,Building2, ArrowRight, Calendar, TrendingUp, BarChart3, PieChart, FileText, Clock, CheckCircle, XCircle, AlertCircle, Video } from "lucide-react";
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{summaryStats.totalCandidates || 0}</p>
              <p className="text-green-600 text-xs mt-1">+{summaryStats.newCandidatesToday || 0} today</p>
            </div>
            <div className="w-14 h-14 bg-gradient-to-r from-slate-100 to-slate-200 rounded-2xl flex items-center justify-center">
              <Users className="w-7 h-7 text-slate-600" />
            </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{summaryStats.totalOpenPositions || 0}</p>
            </div>
            <div className="w-14 h-14 bg-gradient-to-r from-slate-100 to-slate-200 rounded-2xl flex items-center justify-center">
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-gray-200 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 mt-2">{summaryStats.upcomingInterviewsCount || 0}</p>
            </div>
            <div className="w-14 h-14 bg-gradient-to-r from-slate-100 to-slate-200 rounded-2xl flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-6">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-gray-900">Recruitment Funnel</h3>
          </div>
          <div className="h-64">
      ...
      {upcomingInterviews && upcomingInterviews.length > 0 && (
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-8 mb-10">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-white-900">Upcoming Interviews</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcomingInterviews.map((interview) => (
              <div key={interview.interviewId} className="bg-gradient-to-r from-slate-50 to-slate-100 rounded-2xl p-6 border border-slate-200 hover:shadow-lg transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    <div className="w-3 h-3 bg-slate-500 rounded-full"></div>
                    <span className="text-slate-700 font-semibold text-sm">Interview #{interview.interviewId}</span>
                  </div>
        ...
        {recentCandidates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {recentCandidates.slice(0, 6).map((candidate) => (
              <div key={candidate.candidateId} className="bg-slate-50 rounded-2xl p-6 border border-slate-200 hover:shadow-lg transition-all duration-300">
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900">{candidate.candidateName}</h4>
                    <p className="text-gray-600 text-sm">{candidate.candidateEmail}</p>
                  </div>
                </div>
                <div className="mb-3">
                  <p className="text-gray-600 text-sm mb-1">Experience: {candidate.experienceYears} years</p>
                  <p className="text-gray-600 text-sm">Phone: {candidate.candidatePhoneNumber}</p>
                </div>
                <div className="flex flex-wrap gap-1">
                  {candidate.primarySkills?.slice(0, 3).map((skill, index) => (
                    <span key={index} className="px-2 py-1 bg-slate-200 text-slate-700 text-xs rounded-lg">
                      {skill}
                    </span>
                  ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {topCandidatesByExperience.slice(0, 5).map((candidate) => (
              <div key={candidate.candidateId} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
                <div>
                ...
                <div className="text-right">
                  <p className="font-bold text-slate-800">{candidate.experienceYears} years</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-6">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-gray-900">Position Analytics</h3>
          </div>
          <div className="space-y-3">
            {positionAnalytics.map((position) => (
              <div key={position.positionId} className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-bold text-slate-800">{position.totalApplications}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Shortlisted</p>
                    <p className="font-bold text-slate-800">{position.shortlisted}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Selected</p>
                    <p className="font-bold text-slate-800">{position.selected}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {monthlyHiringStats && monthlyHiringStats.length > 0 && (
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-8 mb-10">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Monthly Hiring Statistics</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {monthlyHiringStats.map((stat, index) => (
              <div key={index} className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                <div className="text-center">
                  <p className="text-gray-600 text-sm mb-2">Month {index + 1}</p>
                  <p className="text-3xl font-bold text-slate-800">{stat.count || 0}</p>
          ...
          <div className="space-y-3">
            {topDegrees.map((degree, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="font-medium text-gray-900 text-sm">{degree.degree}</p>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {topUniversities.map((university, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <p className="font-medium text-gray-900 text-sm">{university.university}</p>
                <span className="px-2 py-1 bg-slate-200 text-slate-700 text-xs rounded-full font-semibold">
                  {university.count}
                </span>
              </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
              ...
              <span className="font-bold text-slate-800">{summaryStats.totalSkills || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
              <span className="text-gray-600 text-sm">Total Degrees</span>
              <span className="font-bold text-slate-800">{summaryStats.totalDegrees || 0}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
              <span className="text-gray-600 text-sm">Universities</span>
              <span className="font-bold text-slate-800">{summaryStats.totalUniversities || 0}</span>
            </div>
          </div>
        </div>
      </div>

    ...
    try {
      ...
      if (!searchParam.get("page")) {
        response = await axios.get(
          `http://localhost:8080/candidates/user/${id}`,
          {
            ...
            <div className="p-8 space-y-8">
              <Section title="Personal Information">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InfoField
                    label="First Name"
                    value={candidate.candidateFirstName}
                  />
                  <InfoField
                    label="Middle Name"
                    value={candidate.candidateMiddleName}
                  />
                  <InfoField
                    label="Last Name"
                    value={candidate.candidateLastName}
                  />
                  <InfoField label="Gender" value={candidate.candidateGender} />
                  <InfoField
                    label="Date of Birth"
                    value={formatDate(candidate.candidateDateOfBirth)}
                  />
                  <InfoField
                    label="Phone Number"
              ...
              <Section title="Contact Information">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <InfoField
                    label="Country"
                    value={candidate.candidateCountry}
                  />
                  <InfoField
                    label="Zip Code"
                    value={candidate.candidateZipCode}
                  />
                </div>
              </Section>
              ...
              <Section title="Professional Information">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <InfoField
                    label="Experience"
                    value={`${candidate.candidateTotalExperienceInYears || 0} ${
                      candidate.candidateTotalExperienceInYears === 1
                        ? "Year"
                        : "Years"
                    }`}
                  />
                    ...
                    {candidateSkills.map((candidateSkill, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              candidateSkill.proficiencyLevel === "EXPERT"
                                ? "bg-green-100 text-green-800"
                                : candidateSkill.proficiencyLevel === "ADVANCED"
                        ...
                        <p className="text-sm text-gray-600">
                          ...
                          {candidateSkill.yearsOfExperience === 1
                            ? "year"
                            : "years"}{" "}
                          of experience
                        </p>
